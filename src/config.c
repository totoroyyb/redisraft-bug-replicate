...
static const char *trace_names[] = {
    "off", "node", "conn", "raftlib", "raftlog", "generic", "migration", "all",
};

static const int trace_flags[] = {
    TRACE_OFF,     TRACE_NODE,    TRACE_CONN,      TRACE_RAFTLIB,
    TRACE_RAFTLOG, TRACE_GENERIC, TRACE_MIGRATION, TRACE_ALL,
};
...
static const char *conf_enable_ddb = "enable-ddb";
static const char *conf_ddb_addr = "ddb-addr";
...
static const char *conf_shardgroup_update_interval =
...
const char *err_init =
    "Configuration change is not allowed after init/join for this parameter";
const char *err_raft = "Failed to configure raft: internal error";

static int setRedisConfig(RedisModuleCtx *ctx, const char *param,
                          const char *value) {
  RRStatus ret = RR_OK;
...
static RRStatus updateTLSConfig(RedisRaftConfig *config, RedisModuleCtx *ctx) {
  ...
  SSL_CTX *ssl =
      generateSSLContext(cacert_filename, capath, cert, key, keypass);
  if (!ssl) {
    ret = RR_ERROR;
    goto out;
  }
...
static RedisModuleString *getString(const char *name, void *privdata) {
  ...
  if (strcasecmp(name, conf_log_filename) == 0) {
    ...
    ret = RedisModule_CreateStringPrintf(NULL, "%s:%u", c->addr.host,
                                         c->addr.port);
  ...
  } else if (strcasecmp(name, conf_ddb_addr) == 0) {
    ret = RedisModule_CreateStringPrintf(NULL, "%s", c->ddb_addr.host);
  }
...
bool DDBReportAddrParse(const char *addr, size_t len, DDBReportAddr *report_addr) {
  if (len == 0) {
    return false;
  }
...
static int setString(const char *name, RedisModuleString *val, void *privdata,
                     RedisModuleString **err) {
  RedisRaftCtx *rr = &redis_raft;
  RedisRaftConfig *c = privdata;

  if (rr->state != REDIS_RAFT_UNINITIALIZED) {
    if (!strcasecmp(name, conf_addr) || !strcasecmp(name, conf_slot_config) ||
  ...
  if (strcasecmp(name, conf_log_filename) == 0) {
    ...
    if (*value == '\0') {
      ...
      *err = RedisModule_CreateStringPrintf(
          NULL, "Address is invalid. It must be in the form of 10.0.0.3:8000");
      return REDISMODULE_ERR;
    }
  } else if (strcasecmp(name, conf_ddb_addr) == 0) {
    if (*value == '\0') {
      c->ddb_addr = (DDBReportAddr){0};
    } else if (!DDBReportAddrParse(value, len, &c->ddb_addr)) {
      *err = RedisModule_CreateStringPrintf(
          NULL, "DDB report address is invalid. It must be in the form of 10.0.0.3");
      return REDISMODULE_ERR;
    }
  } else if (strcasecmp(name, conf_slot_config) == 0) {
...
static int setEnum(const char *name, int val, void *privdata,
                   RedisModuleString **err) {
  (void)err;
  (void)privdata;
  RedisRaftCtx *rr = &redis_raft;
...
static int getBool(const char *name, void *privdata) {
  ...
  } else if (strcasecmp(name, conf_enable_ddb) == 0) {
    return c->enable_ddb;
  }
...
static int setBool(const char *name, int val, void *pr,
                   RedisModuleString **err) {
  RedisRaftCtx *rr = &redis_raft;
...
static int validateRedisConfig(RedisModuleCtx *ctx) {
  ...
  if (cfg && *cfg != '0') {
    LOG_WARNING(
        "RedisRaft requires Redis not be started with 'cluster_enabled'!");
...
RRStatus ConfigInit(RedisModuleCtx *ctx, RedisRaftConfig *c) {
  ...
  *c = (RedisRaftConfig){
  ...
  /* clang-format off */
    ...
    ret |= RedisModule_RegisterBoolConfig(ctx,    conf_enable_ddb,                 false,            REDISMODULE_CONFIG_DEFAULT,                 getBool,    setBool,    NULL, c);
...
void ConfigRedisEventCallback(RedisModuleCtx *ctx, RedisModuleEvent eid,
                              uint64_t event, void *data) {
  ...
  for (unsigned int i = 0; i < ei->num_changes; i++) {
    ...
    if (!strcmp(conf, "tls-ca-cert-file") || !strcmp(conf, "tls-ca-cert-dir") ||
        !strcmp(conf, "tls-cert-file") || !strcmp(conf, "tls-key-file") ||
